...
class PackagingTest(unittest.TestCase):
    def test_dashboard_exposes_current_strategy_and_short_extension_status(self):
        ...
        self.assertIn("画像周期盈亏", index_html)
        self.assertIn('id="profile-period-balance"', index_html)
        self.assertIn("画像周期胜率", index_html)
        self.assertIn('id="profile-period-win-rate"', index_html)
        self.assertIn('"profile-period-balance"', app_js)
        self.assertIn('"profile-period-win-rate"', app_js)
        self.assertIn("grid-template-columns: repeat(5, minmax(150px, 1fr));", styles_css)
        ...
        self.assertIn("fmtDirectionPulseShadow", app_js)

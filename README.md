...
- 每日画像选择器默认要求20个独立样本、60%胜率和非负EV，不再用固定数量上限挤出合格画像；已启用画像低于60%胜率、EV不大于0或样本不足时，在当次日评估退出。选中的观察画像可以替代静态时段、动态阈值和技术指标开单资格，但不会绕过持仓、冷却、连续亏损、滚动守卫、画像守卫、金额和滚单规则。
- 默认启用结算序列守卫：某个方向连续3个已结算订单亏损后，仅暂停该方向20分钟。它不预测方向、不反向开仓，也不读取未结订单或被拦订单的未来结果。
...
使用 `--no-daily-profile-selector` 可临时回退到静态主策略。旧观察画像单信号放行参数继续兼容，但每日选择器启用时不再执行旧放行逻辑。

结算序列守卫参数：

- `--no-result-sequence-guard`：关闭守卫。
...
- 每日画像选择替代旧的单信号动态放行；只有完整画像键匹配当日快照才具备开单资格，且不会绕过持仓、冷却、同日三连亏、滚动优势衰退或画像守卫。
- 未结订单达到5单时不再开新单；未满5单且距离上一单不少于2分钟时可以继续开单。
- 新单最小间隔默认2分钟。

...
- `docs/current-strategy.md`：当前策略、指标、开单链路、缓存和运行参数的唯一说明。
- `docs/release-handoff.md`：生产发布历史、当前线上边界、验证结果和后续交接事项。
...
实际订单持久化成功后，Webhook以每个信号一个独立守护线程的方式立即发送。主行情线程不等待网络响应；发送不排队、不重试、不读取响应体，也不记录成功、失败或最近payload。该最低延迟模式明确接受进程退出或网络失败造成的信号丢失。

...
- SHORT 信号额外使用按 `WD/WE + UTC小时` 动态生成的 MACD、RSI、BOLL 阈值确认，避免在恐惧情绪或贴近下轨时追空。
...
- SHORT 比 LONG 更严格：额外提高动态阈值，且必须满足 MACD 柱为负且继续走弱、RSI 未过冷/过热、BOLL 位置没有贴近动态下轨、没有明显下影承接。
...
- 自省不按“某月/某年度低胜率”做永久剔除；低胜率只用于生成开单前可观测的滚动弱化标签。当前实盘模拟已启用滚动守卫：同一 `周期|时段|形态` 最近60天、至少15个已结样本若胜率低于55.56%或EV<=0，则本次不开单。
...
- 高位放量滞涨：仅观察，暂不开单。
- 高位放量下跌：仅在 MACD/RSI/BOLL 与短空时段确认时做空。
- 高位无量上涨：10分钟短线看多。
- 低位缩量下跌：等待。
- 低位放量上涨：仅观察，暂不开单。
- 低位放量承接：仅观察，暂不开单。
...
- MCP 指标复查后，`高位放量下跌` 只在 MACD/RSI/BOLL 与短空专属时段同时确认时开 SHORT；动态指标画像后三个月 SHORT 样本 `5` 单全胜，净值 `+40U`。
- 当前三个月离线回测未引入历史 Fear & Greed 序列，F&G 是实盘风险覆盖层；若要验证历史 F&G 对胜率的影响，需要额外拉取历史 F&G 并按订单时间回放。
- 当前实际开单仍以 LONG 为主，主要有效形态是 `放量急跌反抽`；SHORT 是小样本高确认补充，不放宽为主策略。

...
## 52. 2026-09-02 预热跨月回退修复

### 52.1 根因与修改边界

2026-09-02发布后，服务虽然正常运行，但预热报告为`PARTIAL/87840`：启动目标只请求了2026-08月度ZIP和2026-09-01日度ZIP，没有把服务器已经存在的2026-08-01至2026-08-30日度ZIP用于8月月度文件的回退。由于当前月日度文件在数据源发布前暂不可用，已有历史也被错误地按普通错误降级。

本次只修改`app/history.py`的预热目标选择和状态归类，不修改画像、评分、阈值、方向、开单门禁、订单并发、滚单、守卫、结算、Webhook或数据库结构：

- 完成月份优先使用月度ZIP；月度ZIP缺失时，复用本地同月份已缓存的日度ZIP。
- 当前月份日度ZIP标记为可选补充，缺失仍写入`missing_files/errors`供页面和后续诊断查看，但不阻断已有历史并将其状态降级为`PARTIAL`。
- 历史月的回退日包仍按必需数据处理，缓存文件损坏或没有任何可用回退数据时继续报告实际错误。
- 通过测试验证8月月包缺失时复用日包、当前月日包未发布时历史仍为`READY`，并保留缺失记录。
...
python3 -m unittest discover -s tests
Ran 1176 tests ... OK
python3 -m py_compile app/history.py
bash -n scripts/*.sh
```

...
实现位于 `feature/profile-health-guard` 隔离分支。纯评估器、状态接入、订单/观察持久化、服务参数和启动脚本均采用测试先行。发布顺序固定为：完整验证通过后合并 `main` 并推送，创建并推送发布标签，再从标签构建最小包发布服务器。发布不清空SQLite或现有订单。

实际合并提交、标签、发布目录、重启时间、订单连续性、预热、API状态和验证结果在生产发布完成后回填本节，不另建交接文档。

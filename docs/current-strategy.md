...
6. 如果已完成月份的月度 ZIP 不存在，但本地有该月份的日度 ZIP，则改用这些已缓存的日度文件，不重复请求不存在的月度文件。
7. 当前月份的日度文件属于可选补充数据；下载失败或单个文件损坏不会中断监控，只记录在 `WarmupReport.missing_files` / `WarmupReport.errors`，也不会让已有历史从 `READY` 降级为 `PARTIAL`。
8. 所有加载到的K线按 `open_time` 去重并排序。

历史月份的月包回退日包属于必需历史数据，回退文件损坏或全部缺失时仍会按实际情况报告 `PARTIAL` / `ERROR`。`READY` 表示已有可用历史已完成加载，不代表可选的当前月补充文件全部已发布。
...
发送采用最低延迟模式：订单持久化成功后，每个信号立即启动一个独立守护线程发送POST。行情和开单线程不等待DNS、连接、TLS或HTTP响应；后台线程不排队、不重试、不读取响应体，也不记录payload、成功时间、错误或返回值。进程退出或网络异常可能导致信号丢失，这是当前模式接受的边界。`--no-webhook` 仍可完全关闭发送。
...
- 该节点当时会在状态接口保留并脱敏最近payload；当前最低延迟版本不再保留任何payload或发送结果。
...
页面顶部同时展示累计表现和北京时间交易日表现。累计字段继续使用全部模拟订单；“今日盈亏”和“今日胜率”以北京时间08:00为日界线，只统计 `settled_at` 落在 `[当日08:00, 次日08:00)` 的已结算订单。08:00前查看时使用前一日08:00开始的交易日，按结算时间归属，不按开单时间归属，未结订单不进入今日胜率或收益。

`/api/state.stats.today` 返回：
...
没有已激活画像时，`active=false`，版本为空，生效时间为 `null`，收益、订单数和胜率均为0；页面显示 `-`。`by_direction_slot` 保留兼容汇总，`by_direction_slot_scope` 用 `LEGACY_GLOBAL_V1` 和 `DIRECTION_V2` 隔离并发位置口径。画像周期统计用于评价当前DPS版本；它额外要求订单DPS版本一致，因此不能替代不限制画像版本的08:00交易日统计，也不参与开单、结算、每日画像选择或任何风险守卫。
...
持续影子优化器默认关闭，只有 `SHADOW_OPTIMIZER=1` 或 `--enable-shadow-optimizer` 才启动。它不替换现有10分钟候选生成，也不把影子订单发送到Webhook。正式行情更新和正式SQLite提交成功后，行情协调器才把同一批闭合1分钟K线非阻塞投递到独立CPU进程；默认队列深度120。队列满、子进程异常或影子数据库失败只登记影子缺口，不回滚正式订单和游标。

当前自动优化范围只包含 `ProfileAdmissionPolicy` 画像准入参数族。每个交易对同时运行一个 Champion 和最多7个 Challenger；每组拥有独立的 `MonitorState`、模拟订单、LONG/SHORT容量、开单间隔、金额资格、画像状态、守卫状态和事件游标。完整运行配置和参数使用不可变规范JSON及SHA-256版本化。相同`analyzer_hash`的组共享一次只读原始量价/指标/观察候选计算，各组仍独立执行画像准入、守卫、状态迁移和订单结算；REST补偿批次与正式路径一致，只批量更新一次并以批次最后一分钟推进游标。

影子账本固定使用独立 `monitor.shadow.sqlite3`，schema V3保存参数快照、实验、arm、分钟游标、缺口、逐分钟决定、影子订单、观察记录、日汇总、决策日聚合、评估、晋级/回退历史、正式策略回执和恢复状态。影子订单、参数、正式回执及生命周期永久保留；普通未开单分钟明细30天后先按北京时间交易日、方向、画像和首个阻止原因聚合，再清空大载荷。容量分级为4GiB告警、4.5GiB停止普通可选明细、5GiB页上限，并为核心记录预留512MiB；新库启用增量回收并分别报告有效、物理和可回收空间。

每天北京时间07:50只使用当时已经结算且连续覆盖的前向样本评估，08:00处理首名候选。晋级必须同时满足：7个完整无缺口日、至少300笔、总胜率至少60%、LONG和SHORT分别至少55.56%、至少5个合格胜率日和5个正EV日、相对Champion至少提高2个百分点且至少5天胜出、日均单量不少于 `max(35, Champion的70%)`，最长连亏和最大回撤不劣于Champion。排序依次比较总胜率、较弱方向胜率、最差滚动3日胜率、相对增量、与日均50笔的距离、连亏、回撤、EV/PnL、复杂度和参数哈希。

7日内最多晋级一次。正式进程收到独立生命周期通道请求后，先校验交易对、代次、实验、arm和参数哈希；若无在途订单立即替换画像准入策略，否则等待全部结算，最长10分钟，只影响后续新订单。影子侧在正式应用前不修改Champion；正式策略不可变回执持久化成功并返回ACK后，影子侧才提交晋级/回退，拒绝或过期请求不改变实验状态。启用影子优化器时，重启只从已确认回执恢复正式策略；关闭影子优化器时仍以启动参数为正式事实。正式种子已领先旧实验游标时创建替代实验，避免停机后全组永久冻结。原Champion继续运行14天作为同期回退对照。新Champion连续8亏、50笔胜率低于50%、100笔胜率低于55.56%且落后旧版至少5个百分点，或双方同期均至少20笔且最大回撤超过旧版1.25倍时回退；失败版本标记 `RETIRED`，不再消费事件或参与晋级。

轻量状态位于 `/api/state.shadow_optimizer` 和 `/api/shadow`，实验及完整参数通过 `/api/shadow/experiment` 读取，影子订单通过 `/api/shadow/orders?arm_id=...&page=1&page_size=20` 真分页读取。切换币种会取消尚未生效的旧币种参数请求，并用新币种代次和预热快照重建影子实验。
